...

{
    ...
    is(mop::class_for($foo)->get_slot_at( $foo, '$bar' ), 100, '... got the expected initial value');
}

{
    ...
    ok($foo->is_a( Foo ), '... this is a Foo');
    is(mop::class_for($foo), Foo, '... this is a Foo');
    is(mop::class_for($foo)->get_slot_at( $foo, '$bar' ), 200, '... got the expected initial value');
}
